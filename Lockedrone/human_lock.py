...
def main():
    ...
    # Points to your optimized CPU/NCNN model directory
    ...
    try:
        while True:
            ...
            # 3. Perform human tracking inference (Downscaled to 256 for maximum Pi performance)
            target = detector.detect_primary_target(frame, conf_threshold=0.40, imgsz=256)
            ...
            if target:
                ...
                # 5. Output telemetry vectors (These will link directly to your flight controller)
                print(f"[TRACKING] FPS: {current_fps:.1f} | Conf: {confidence:.2f} | "
                      f"Dist to Target: {distance_to_target:.1f}cm | Ground Alt: {distance_to_ground:.1f}cm | "
                      f"Center Offset X: {horizontal_offset:.1f}px")
                
                # Optional visual overlay for bench-testing (Disable when flying to save CPU cycles)
