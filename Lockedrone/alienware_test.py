...
def main():
    print("[INFO] Initializing Locke Drone PC Prototyping Environment...")
    
    # Setup for Windows Desktop testing
    cam = BackgroundCamera(src=0, width=640, height=480)
    
    # CHANGED: We swap the rigid NCNN directory out for the flexible desktop PyTorch model.
    # The framework will automatically download 'yolo11n.pt' on its first run.
    ...
    print("[INFO] PC Environment armed. Camera active.")
    time.sleep(1.0) 
    ...
    try:
        while True:
            # Gives Windows window manager time to breathe
            time.sleep(0.01)
            ...
            # Desktop PyTorch inference engine
            ...
            if target:
                ...
                # Render tracking visual overlays
            ...
            if fps_counter % 10 == 0:
                clear_screen()
                print("================ LOCKE DRONE PC DEVELOPER MODE ================")
            ...
            # 30ms window delay lets Windows clear its graphics buffer seamlessly
            if cv2.waitKey(30) & 0xFF == ord('q'):
                print("\n[INFO] Exiting development environment safely...")
                break

    except KeyboardInterrupt:
        print("\n[INFO] Interrupted via terminal command.")
        
    finally:
        cam.release()
        cv2.destroyAllWindows()
        print("[INFO] PC testing engine offline.")
